...
def pco2_tnorm(pCO2_obs,temp_obs):
    ...
    pCO2_NT = pCO2_obs * exp(0.0423 * (Tmean - T_obs))     (eqn 1)
    ...
    pCO2_T = pCO2_mean * exp(0.0423 * (Tobs - Tmean))     (eqn 2)
    ...
    'pCO2_AM': long-term mean observed pCO2 (uatm)
    ...
    temp_AM = np.nanmean(temp_obs)
    pCO2_NT = pCO2_obs * np.exp(0.0423 * (temp_AM - temp_obs))

    # - - -
    # pCO2_T = thermal component pCO2 (uatm)
    pCO2_AM = np.nanmean(pCO2_obs)
    pCO2_T = pCO2_AM * np.exp(0.0423 * (temp_obs - temp_AM))

    # - - -
    # pCO2_Tanom = thermal pCO2 component  seasonal cycle anomaly (uatm)
    pCO2_Tanom = pCO2_T - pCO2_AM
...
def pco2_fass(**kwargs):
    ...
    # - - -
    # Step 2: solve for long-term average pCO2_AM (eqn 4 in Rodgers et al)
